...
export class QueriesControl extends Control<{}> {
    ...
    private async updateQueries() {
        ...
        queries.push(new Query("Children tree (only open and assigned to me)","bowtie-work-item-bug", "SELECT [System.Id],[System.WorkItemType],[System.Title],[System.AssignedTo], [System.State],[System.Tags] FROM workitemLinks WHERE ([Source].[System.TeamProject] = @project AND [Source].[System.Id] = {{id}}) AND ([System.Links.LinkType] = 'System.LinkTypes.Hierarchy-Forward') AND ([Target].[System.TeamProject] = @project AND [Target].[System.WorkItemType] <> '' AND [Target].[System.State] <> 'Closed' AND [Target].[System.State] <> 'Resolved' AND [Target].[System.AssignedTo] = @me) MODE (Recursive)"));
        if (this.parent) {
